...
function leaders(person){
   ...
   return null; // gives back undefined for some reason??
}
...
Declare another variable named `copyValuesArray` and assigning its value to an empty array.
...
var topQuote = 'If can, can';

function longestWord(str){
   var wordArr = str.split(' ');
   var current = "";
   for(i=0; i<wordArr.length; i++){
      if(wordArr[i].length > current.length){
         current = wordArr[i];
      }
   }
   return current;
...
function graduateAndSetNewClass(cohort){
   for(i=0; i<cohort.length; i++){
      if(cohort[i].enrolled === true){
         cohort[i].graduated = true; // === and == only for conditional statements. duh
      }else{
